...
internal static class Program
{
  ...
  */
  ...
  static void camera_rotate_around_axis(ref Camera c, Vector axis, double radians)
  {
    ...
    Al.RotateTransform3D(ref t, axis.x, axis.y, axis.z, (float)radians);
    Al.TransformCoordinates3D(ref t, ref c.yaxis.x, ref c.yaxis.y, ref c.yaxis.z);
    Al.TransformCoordinates3D(ref t, ref c.zaxis.x, ref c.zaxis.y, ref c.zaxis.z);
  ...
  */
  static void setup_3d_projection()
  {
    ...
    double f;
    Al.IdentityTransform(ref projection);
    Al.TranslateTransform3D(ref projection, 0, 0, -1);
    f = Math.Tan(ex.camera.vertical_field_of_view / 2);
    Al.PerspectiveTransform(ref projection, (float)(-1 * dw / dh * f), (float)f,
       1,
       (float)(f * dw / dh), (float)-f, 1000);
    Al.UseProjectionTransform(ref projection);
  }
  ...
  {
    int x, y;
    AllegroColor c1 = Al.ColorName("yellow");// Al.MapRgb(0, 255, 255); //yellow
    AllegroColor c2 = Al.ColorName("green");// Al.MapRgb(0, 255, 0); //al_color_name("green");
  ...
  */
  static void add_skybox()
  {
    ...
    AllegroColor c1 = Al.ColorName("black");// Al.MapRgb(0, 0, 0); // al_color_name("black");
    AllegroColor c2 = Al.ColorName("blue");// Al.MapRgb(0, 0, 255); // al_color_name("blue");
    AllegroColor c3 = Al.ColorName("white");// Al.MapRgb(255, 255, 255); // al_color_name("white");
  ...
  static void draw_scene()
  {
    ...
    Al.SetRenderState(AllegroRenderState.DepthTest, 1); //al_set_render_state(ALLEGRO_DEPTH_TEST, 1);
    ...
    Al.DrawPrim(ex.v, ex.skybox, 0, ex.n, PrimType.TriangleList);
    ...
    var cx = ex.camera.zaxis.x;
    var cy = ex.camera.zaxis.y;
    var cz = ex.camera.zaxis.z;
    Al.DrawText(ex.font, front, 0, th * 0, 0, $"look: {cx:3.1}/{cy:3.1}/{cz:3.1} (change with left mouse button and drag)");
    ...
    Al.DrawText(ex.font, front, 0, th * 1, 0, $"pitch: {pitch:4.0} yaw: {yaw:4.0} roll: {roll:4.0}");
    Al.DrawText(ex.font, front, 0, th * 2, 0, $"vertical field of view: {(ex.camera.vertical_field_of_view * 180 / Al.ALLEGRO_PI):3.1} (change with Z/X)");
    Al.DrawText(ex.font, front, 0, th * 3, 0, "move with WASD or cursor\")");
    Al.DrawText(ex.font, front, 0, th * 4, 0, $"control style: {ex.controls_names[ex.controls]} (space to change)");
  }
    ...
    while (true)
    {
      ...
      else if (@event.Type == EventType.KeyDown)
      {
        if (@event.Keyboard.KeyCode == KeyCode.KeyEscape)
          break;
        if (@event.Keyboard.KeyCode == KeyCode.KeySpace)
        {
          ex.controls++;
          ex.controls %= 3;
        }

        ex.key[(int)@event.Keyboard.KeyCode] = 1;
        ex.keystate[(int)@event.Keyboard.KeyCode] = 1;
      }
      else if (@event.Type == EventType.KeyUp)
        ex.keystate[(int)@event.Keyboard.KeyCode] = 0;
      ...
      {
        ex.mouse_dx += @event.Mouse.X;
        ex.mouse_dy += @event.Mouse.Y;
      }
