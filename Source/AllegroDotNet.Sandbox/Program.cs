...
internal static class Program
{
  ...
  public static void Main(string[] args)
  {
    ...
    Console.WriteLine($"Is Allegro installed: {Al.IsSystemInstalled()}");
    Console.WriteLine($"Init Allegro: {Al.InstallSystem(AllegroLibraryVersion.V527, null)}");
    Console.WriteLine($"Is Allegro installed: {Al.IsSystemInstalled()}");

    Console.WriteLine($"Is Image Addon installed: {Al.IsImageAddonInitialized()}");
    Console.WriteLine($"Init Image Addon: {Al.InitImageAddon()}");
    Console.WriteLine($"Is Image Addon installed: {Al.IsImageAddonInitialized()}");

    Al.InstallKeyboard();

    var timer = Al.CreateTimer(1.0 / 60.0);
    Al.StartTimer(timer);
    Al.Rest(0.5);
    Al.StopTimer(timer);
    Console.WriteLine($"500ms is how many ticks?: {Al.GetTimerCount(timer)}");
    
    Al.SetNewDisplayFlags(DisplayFlags.Windowed | DisplayFlags.Resizable);
    var d = Al.CreateDisplay(1280, 720);
    
    Al.ClearToColor(Al.MapRgb(32, 64, 128));
    Al.FlipDisplay();
    

    var queue = Al.CreateEventQueue();
    Al.RegisterEventSource(queue, Al.GetDisplayEventSource(d));
    ...
    Al.RegisterEventSource(queue, myUserEventSource);
    var myUserEvent = new AllegroEvent
    {
      Type = EventType.TouchCancel,
      User =
      {
        Data1 = IntPtr.MinValue,
        Data2 = IntPtr.MaxValue,
        Data3 = IntPtr.MinValue,
        Data4 = IntPtr.MaxValue
      }
    };
    ...
    while (true)
    {
      ...
      if (allegroEvent.Type == EventType.DisplayResize)
        Al.AcknowledgeResize(d);

      if (allegroEvent.Type == EventType.TouchCancel)
        allegroEvent.Type = EventType.DisplayExpose;

      if (allegroEvent.Type == EventType.Timer)
      {
        Al.GetKeyboardState(keyboardState);
        if (Al.KeyDown(keyboardState, KeyCode.KeyEscape))
          break;
        
        Al.ClearToColor(Al.MapRgb(128, 64, 255));
        Al.FlipDisplay();
      }

      if (allegroEvent.Type == EventType.DisplayResize && allegroEvent.Source == d)
        Al.AcknowledgeResize(d);
    }
